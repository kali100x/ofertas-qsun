...
GREY_TOTAL = "DFF3D6"  # fondo fila Total (verde claro)
BORDER = "000000"      # bordes negros finos, bien definidos
FONT = "Times New Roman"  # tipografia clasica (serif)
...
def generate(data, out_path):
    ...
    sub.paragraph_format.space_after=Pt(20)   # aire antes de los cuadros
    ...
    set_widths(pt, [2.5, 6.0, 0.6, 2.5, 6.0], fixed=True)   # 17.6 cm
    cell_margins(pt, top=26, bottom=26, left=120, right=120)   # filas mucho mas compactas
    ...
    for i in range(6):
        ...
        set_cell(pt.rows[i].cells[1], valL[i], size=10, bold=(i==0))
        ...
        for c in (0,1,3,4): cell_borders(pt.rows[i].cells[c])   # bordes negros solo en los dos cuadros
        row_height(pt.rows[i], 470 if i==1 else 300)           # 1 linea ~6-7mm; 2 lineas ~9-10mm
    para(d,"",after=16)   # separacion con Detalle del suministro
    ...
    for j,h in enumerate(hdr):
        set_cell(tb.rows[0].cells[j],h,bold=True,size=9,align="center")   # cabecera clara (sin fondo)
    ...
    set_widths(tb, [3.6, 2.2, 1.9, 2.5, 1.8, 2.4, 3.2], fixed=True)   # importe ancho para cifras grandes
    r0=1+n_prod
    for i in range(n_sum):
        r=tb.rows[r0+i]; istotal=(i==2)
        set_cell(r.cells[0], slabels[i], size=10, bold=istotal, align="left")     # Total sin fondo, solo negrita
        set_cell(r.cells[6], (svals[i]+" €" if svals[i]!="—" else svals[i]), size=10, bold=istotal, align="right")
    ...
    for k,rr in enumerate(tb.rows):
        ...
        elif k<1+n_items: h=580              # fila(s) de producto (mas protagonista)
        elif k<1+n_prod: h=400               # fila(s) en blanco
        else: h=260                          # resumen economico
        row_height(rr, h)
    tbl_borders(tb)

    # producto y garantias (primera hoja, tras el resumen)
    section_title(d, tr["prod_title"], after=6, before=16)   # mas separacion respecto a la tabla
    prod_prefix = f"{lines[0]['item']} ({nint(lines[0]['peak'])} W) " if lines else ""
    prod_line = (prod_prefix + str(data.get("descripcion") or "")).strip()
    if prod_line: para(d, prod_line, size=10.5, after=6)     # modelo (Wp) + descripcion, misma linea, sin negrita
    ...
    if wr:
        ...
        set_widths(wt, [17.6/len(wr)]*len(wr), fixed=True)
        cell_margins(wt, top=55, bottom=55, left=110, right=110)
        for rr in wt.rows: row_height(rr, 480)
        tbl_borders(wt)
    para(d,"",after=6)
    para(d, tr["w_note"], size=10.5, after=2)                                   # nota de garantias
    ...
    set_widths(bt, [4.6, 13.0], fixed=True)   # recuadro a todo el ancho, valores alineados
    ...
    cells=[(tr["a_name"],tr["a_sign"]),(tr["a_date"],tr["a_pos"])]
    ...
    set_widths(at, [8.8, 8.8], fixed=True)
